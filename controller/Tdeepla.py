...
def train(lr=1e-3,epochs=250,path="./checkpoints/deepla2"):
    set_seed()
    device=get_device()
    train_loader,test_loader=init_data()
    model=DeepLA_PointMLP_Model(embed_dim=64,num_classes=40).to(device)
...
def eval(path="./checkpoints/deepla"):
    # ----------- 加载模型权重 -----------
