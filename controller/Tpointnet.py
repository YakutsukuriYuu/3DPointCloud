...
def one_epoch_train(model,criterion,optimizer,dataloader,device):
    ...
    for x,y in tqdm(dataloader):
        ...
        y=y.to(device)
        y_hat,trans,trans_feat=model(x)
...
def train(lr=1e-3,epochs=250,path="./checkpoints/pointnet"):
    set_seed()
    device=get_device()
    model=PointNetCls(k=40,feature_transform=True).to(device)
...
def eval(path="./checkpoints/pointnet"):
    # ----------- 加载模型权重 -----------
